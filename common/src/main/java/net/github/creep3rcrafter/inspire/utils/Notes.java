package net.github.creep3rcrafter.inspire.utils;

public class Notes {
    /*
    Sea Moss
    Wet Mossy Sand
    Wet Sand Bricks
    Ice Brick
    Snow Brick
    Sand Pile
    Gravel Pile
    Wet Sand Pile
    Alge
    Duckweed
    Lilypads
    Trial Timer
    Grapple Shot
    Boomerang
    Collection Enchantment
    Reach Enchantment
    Sticky Redstone
    Crates of Stuff
    Torch Arrow
    Block Arrow
    Color Sensor
    Horse Chest
    Undead Horse Armor
    Large Candles
    Wax Block
    Random Houses
    Flooded Village
    Ice Castle
    Underwater Trials
    Enchantment Holding
    Lavalogged Blocks
    Unstable Snow
    Enchantment Ricoshe
    Useful Trims

    Tree Seeds Spruce(Pinecone), (Apple Tree)Seed, Acorn
    Spruce Cone used to plant saplings or make tea
    Apple Tree Seed
    Birch Tree Seed falls and spins
    Acorn small animals want it
    cherry
    cherry pit
    acacia seed packet

    Apple Tree
    Golden Apple Tree

    leaf wall / Hedges

    potion enchantments
    life steal enchantment
    attack speed enchantment/swift for bows
    lightning enchantment

    large raw gold nugget
    raw gold nugget
    cobble = gravel, sand, gold nuggets
    crusher crushes blocks like stone, netherrack, cobblestone, gravel, for nuggets
    Ideas
    Honey fluid
    skeleton dog
    skeleton creeper
    skeleton enderman
    skeleton bear
    skeleton illager
    wither skeleton dog
    wither skeleton creeper
    wither skeleton enderman
    wither skeleton bear
    wither skeleton illager
    willager evil illager type that experaments on creatures and bones to make wither creatures
    soul lily pad
    sculk lily pad
    combo ores that have more than one type of ingot in them
    mirror uses a camera and rendertexture
    dile
    controlled dile
    controlled lever
    filter
    giant versions of all trees
    soul gravel
    soul stone
    soul cobblestone
    soul grass
    soul flint
    soul flint and netherite to open ether portal? lights soul fire candles and blocks
    soul dragon purifys sculk and has 3 heads
    gillagers underwater villagers use prismarine arrows and tridents
    gillage village coral castles and towers with bluestone
    baked pumpkin seeds
    white pumpkins
    red pumpkins
    green pumpkins
    golden crown trade with piglin like villagers
    netherite crown like gold crown
    travel stone ender perls and blaze powder to teleport
    miners compass
    smelting tablet
    ender box/bag
    redstone heart
    bucket of void
    bucket of oceans
    bedrock shard
    conjering table make spells
    spellbooks freeze, burn, shock, plage, knockback, damage, (cone ray sphere(cube), point)
    netherite scrap armor
    netherite scrap tools
    sulfer ore
    sulfercane
    nether star block bedrock netherstars
      stuck in the nether roof that look like stars



    Missing Textures / Models / Item Properties-------------------------------
    Jar of Beetroot soup
    Jar of Carrot soup
    Jar of Potato soup
    Jar of Beef Stew
    Jar of Chicken Stew
    Jar of Mutton Stew
    Jar of suspicious Stew
    Jar of mushroom Stew
    Jar of rabbit Stew
    glob blocks
    music disc an ordinary day
    music disc ancestry
    music disc hal4
    music disc infinite amethyst
    music disc left to bloom
    music disc nuance1
    music disc nuance2
    music disc one more day
    music disc piano1
    music disc piano2
    music disc piano3
    music disc stand tall
    music disc wending
    withered sign
    withered trapdoor
    redwood sign
    cheese
    corn
    baked corn
    popped corn
    caramel corn
    dried corn
    iron pickaxe head
    iron axe head
    iron shovel head
    iron hoe head
    iron sword blade

    iron pickaxe head
    iron axe head
    iron shovel head
    iron hoe head
    iron sword blade

    gold pickaxe head
    gold axe head
    gold shovel head
    gold hoe head
    gold sword blade

    diamond pickaxe head
    diamond axe head
    diamond shovel head
    diamond hoe head
    diamond sword blade

    netherite pickaxe head
    netherite axe head
    netherite shovel head
    netherite hoe head
    netherite sword blade

    crafting tablet
    wand of light

    pine wood set
    redwood wood set

    Missing 3d item models-------------------------------
    spears
    clubs

    Missing ore generation-------------------------------
    bluestone ore
    deepslate blustone ore
    amethyst ore
    deepslate amethyst ore
    prismarine ore
    deepslate prismarine ore
    Missing code-------------------------------
    withered bone meal
    spears
    crafting tablet
    wand of light
    dried corn
    iron pickaxe head
    iron axe head
    iron shovel head
    iron hoe head
    iron sword blade

    iron pickaxe head
    iron axe head
    iron shovel head
    iron hoe head
    iron sword blade

    gold pickaxe head
    gold axe head
    gold shovel head
    gold hoe head
    gold sword blade

    diamond pickaxe head
    diamond axe head
    diamond shovel head
    diamond hoe head
    diamond sword blade

    netherite pickaxe head
    netherite axe head
    netherite shovel head
    netherite hoe head
    netherite sword blade
     */
}
