...
public class ThrownDynamite extends ThrowableItemProjectile {
    public ThrownDynamite(EntityType<? extends ThrownDynamite> entityType, Level level) {
        super(entityType, level);
    }

    public ThrownDynamite(Level level, LivingEntity livingEntity) {
        super(InspireEntityTypes.DYNAMITE.get(), livingEntity, level);
    }

    public ThrownDynamite(Level level, double x, double y, double z) {
        super(InspireEntityTypes.DYNAMITE.get(), x, y, z, level);
    }

    @Override
    protected @NotNull Item getDefaultItem() {
        return InspireItems.DYNAMITE.get();
    }
    ...
    protected void onHitBlock(BlockHitResult blockHitResult) {
        ...
        if (!this.level.isClientSide) {
            Utils.explode(this.getLevel(), blockHitResult.getBlockPos(), 2f);
            discard();
        }
    }
}
